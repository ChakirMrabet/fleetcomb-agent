...
public static class ReportHealth
{
    ...
    public sealed class Validator : AbstractValidator<Command>
    {
        public Validator()
        {
            ...
            RuleFor(x => x.Severity).Must(x => x is "Healthy" or "Degraded" or "Unhealthy");
        }
    }
        ...
        : IRequestHandler<Command, ProducerMessage>
    {
        public Task<ProducerMessage> Handle(Command request, CancellationToken token) =>
            messages.SubmitAsync(
                request.AdapterId, "health", request.Schema, request.Severity,
